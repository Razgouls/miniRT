...
/*   Updated: 2020/12/09 10:48:44 by eoliveir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_printf_error_colors(char **strs, char **colors, int index)
{
	...
	while (colors[i])
	{
		if (i > 2)
		{
			ft_printf("Error n : Trop de couleurs dans RGB : %s\n", strs[3 + index]);
			return (1);
		}
		if ((int)ft_atof(colors[i]) < 0 || (int)ft_atof(colors[i]) > 255)
		{
			ft_printf("Error n : Les valeurs RGB sont erronées : %s\n", strs[3 + index]);
			return (1);
		}
		i++;
	}
...
int		ft_printf_error_origin(char **origin, int index)
{
	...
	if (i < 3)
	{
		ft_printf("Error n : origindonnées manquantes : ");
		while (++j < i)
	...
	if (i > 3)
	{
		ft_printf("Error n : Trop de origindonnées : ");
		while (++j < i)
			ft_printf("%s,", origin[j]);
		ft_printf("\n");
		(index == 0) ? ft_free_tab(origin) : NULL;
...
int		ft_printf_error_vector(char **origin)
{
	...
	while (origin[i])
	{
		...
		{
			ft_printf("Error n : Vecteur : x, y, z doit être compris entre -1 et 1 : ");
